"""Unit tests for milestones router."""
...
@pytest.mark.parametrize(
    ...
    [
        ("/api/v1/milestones", "milestones", "View pending and achieved milestones."),
        ("/api/v1/milestoneTypes", "milestoneTypes", "View available milestoneType options"),
        (
            "/api/v1/milestoneStatistics",
            "milestoneStatistics",
            "View available milestone statistics options",
        ),
        ("/api/v1/milestoneLookups", "milestoneLookups", "View available milestoneLookup options"),
        (
...
def test_milestones_endpoint_exists(
    client: TestClient, endpoint_path: str, operation_id: str, summary: str, respx_mock
):
    """Test that milestones endpoint exists and returns expected structure."""
...
@pytest.mark.unit
def test_milestones_endpoints_require_valid_upstream(client: TestClient, respx_mock):
    """Test that milestones endpoints handle upstream failures gracefully."""
...
@pytest.mark.unit
def test_milestones_endpoints_forward_query_params(client: TestClient, respx_mock):
    """Test that milestones endpoints forward query parameters correctly."""
