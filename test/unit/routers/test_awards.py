...
@pytest.mark.parametrize(
    ...
    [
        ("/api/v1/awards/123/recipients", "awardRecipients", "View recipients of an award"),
        ("/api/v1/awards", "awards", "View awards info"),
        ("/api/v1/awards/123", "awards_1", "View awards info"),
    ],
)
def test_awards_endpoint_exists(
    client: TestClient, endpoint_path: str, operation_id: str, summary: str, respx_mock
):
    """Test that awards endpoint exists and returns expected structure."""
...
@pytest.mark.unit
def test_awards_endpoints_require_valid_upstream(client: TestClient, respx_mock):
    """Test that awards endpoints handle upstream failures gracefully."""
...
@pytest.mark.unit
def test_awards_endpoints_forward_query_params(client: TestClient, respx_mock):
    """Test that awards endpoints forward query parameters correctly."""
