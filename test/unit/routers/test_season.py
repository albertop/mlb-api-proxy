...
@pytest.mark.parametrize(
    ...
    [
        ("/api/v1/seasons/all", "allSeasons", "View all seasons"),
        ("/api/v1/seasons", "seasons", "View season info"),
        ("/api/v1/seasons/123", "seasons_1", "View season info"),
    ],
)
def test_season_endpoint_exists(
    client: TestClient, endpoint_path: str, operation_id: str, summary: str, respx_mock
):
    """Test that season endpoint exists and returns expected structure."""
...
@pytest.mark.unit
def test_season_endpoints_require_valid_upstream(client: TestClient, respx_mock):
    """Test that season endpoints handle upstream failures gracefully."""
...
@pytest.mark.unit
def test_season_endpoints_forward_query_params(client: TestClient, respx_mock):
    """Test that season endpoints forward query parameters correctly."""
